...
if (
  ((!argv.host || !argv.port || !argv.key || !argv.cert) && !argv.dumb) ||
  argv.help ||
  (type !== "full" && type !== "light" && type !== "nano")
) {
  console.log(
    "Nimiq NodeJS client\n" +
      "\n" +
...
if (argv["log"]) {
  Nimiq.Log.instance.level =
    argv["log"] === true ? Nimiq.Log.VERBOSE : argv["log"];
}
if (argv["log-tag"]) {
...
console.log(
  `Nimiq NodeJS Client starting (${
    host && port ? `host=${host}, port=${port}` : "dumb"
  }, miner=${!!minerOptions}, statistics=${!!statisticsOptions}, passive=${!!passive}, rpc=${!!rpc})`
);
...
(async () => {
  ...
  Nimiq.Log.i(
    TAG,
    `Peer address: ${networkConfig.peerAddress.toString()} - public key: ${networkConfig.keyPair.publicKey.toHex()}`
  );
  ...
  Nimiq.Log.i(
    TAG,
    `Managing wallets [${addresses.map(address =>
  ...
  Nimiq.Log.i(
    TAG,
    `Wallet initialized for address ${$.wallet.address.toUserFriendlyAddress()}.` +
      (!isNano
        ? ` Balance: ${Nimiq.Policy.satoshisToCoins(account.balance)} NIM`
        : "")
  );

  $.miner = new Nimiq.Miner(
    $.blockchain,
    $.accounts,
    $.mempool,
    $.network.time,
  ...
  if (minerOptions) {
    ...
    if (typeof minerOptions === "number") {
      ...
      if (margs.length >= 2) {
        $.miner.throttleAfter = parseInt(margs[1]) * $.miner.threads;
      }
      if (margs.length >= 3) {
        $.miner.throttleWait = parseInt(margs[2]);
      }
    }
    if (passive) {
      $.miner.startWork();
  ...
  $.consensus.on("established", () => {
    Nimiq.Log.i(
      TAG,
      `Blockchain ${type}-consensus established in ${(Date.now() - START) /
        1000}s.`
    );
    Nimiq.Log.i(
      TAG,
      `Current state: height=${$.blockchain.height}, totalWork=${
        $.blockchain.totalWork
      }, headHash=${$.blockchain.headHash.toBase64()}`
    );
  });
  ...
  if (statisticsOptions) {
    ...
    const outputInterval =
      typeof statisticsOptions === "number" ? statisticsOptions : 10; // seconds
    ...
    $.miner.on("hashrate-changed", async hashrate => {
      ...
      if (hashrates.length >= outputInterval) {
        ...
        Nimiq.Log.i(
          TAG,
          `Hashrate: ${(sum / hashrates.length)
            .toFixed(Math.log10(hashrates.length))
            .padStart(7)} H/s` +
            (!isNano
