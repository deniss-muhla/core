class Miner extends Observable {
  ...
  async _startWork() {
    ...
    try {
      ...
      Log.i(
        Miner,
        `Starting work on ${block.header}, transactionCount=${
          block.transactionCount
        }, hashrate=${this._hashrate} H/s`
      );

      this._workerPool
        .startMiningOnBlock(
          block,
          this._shareCompactSet ? this._shareCompact : undefined
        )
        .catch(Log.w.tag(Miner));
    } catch (e) {
  ...
  async _onWorkerShare(obj) {
      ...
      if (!this._submittingBlock) {
        ...
        if (BlockUtils.isProofOfWork(obj.hash, obj.block.target)) {
          ...
          if (await obj.block.header.verifyProofOfWork()) {
            ...
            Log.d(
              Miner,
              `Ignoring invalid share: ${await obj.block.header.pow()}`
            );
          }
        }
    ...
    if (
      ...
      this._lastRestart + Miner.MIN_TIME_ON_BLOCK < Date.now()
    ) {
      this._startWork().catch(Log.w.tag(Miner));
    }
  }
  ...
  async getNextBlock() {
    ...
    try {
      ...
      if ((await this._blockchain.getNextTarget()) !== nextTarget)
  ...
  async _getNextHeader(nextTarget, interlink, body) {
    ...
    let accountsHash;
    try {
      await accounts.commitBlockBody(
        body,
        height,
        this._blockchain.transactionCache
      );
  ...
  async _getNextBody(interlinkSize) {
    ...
    const prunedAccounts = await this._accounts.gatherToBePrunedAccounts(
      transactions,
      this._blockchain.height + 1,
      this._blockchain.transactionCache
    );
